...
const LeaderboardEntry = ({ entry, rank, teams, results, isExpanded, onToggle }) => {
  const { name, score, maxPossible } = entry;
  ...
  return (
    <div
      className={`
        ...
        ${isExpanded ? 'ring-2 ring-green-500 shadow-md' : 'hover:shadow-md hover:border-gray-300'}
      `}
      onClick={onToggle}
    >
      <div className="flex items-center p-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold truncate text-gray-900">{name}</div>
...
export const Leaderboard = ({ entries, results, teams }) => {
  const leaderboard = useLeaderboard(entries, results, teams);
      ...
      {leaderboard.map((entry, index) => (
        <LeaderboardEntry
          ...
          onToggle={() => setExpandedEntry(
            expandedEntry === entry.name ? null : entry.name
          )}
        />
      ))}
    </div>
